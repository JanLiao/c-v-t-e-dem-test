...
public class TreeTableDemo extends Application {
    ...
    public void start(Stage primaryStage) throws Exception {
        ...
        JFXTreeTableColumn<User, Object> btnColumn = new JFXTreeTableColumn<>("Button");
        btnColumn.setPrefWidth(150);
        btnColumn.setCellValueFactory((TreeTableColumn.CellDataFeatures<User, Object> param) -> {
            if (btnColumn.validateValue(param)) {
                return param.getValue().getValue().btn;
            } else {
                return btnColumn.getComputedValue(param);
            }
        });
        ...
        users.add(new User(COMPUTER_DEPARTMENT, "23", "CD 1", new JFXButton("nihao")));
        users.add(new User(SALES_DEPARTMENT, "22", "Employee 1", new JFXButton("nihao")));
        users.add(new User(SALES_DEPARTMENT, "24", "Employee 2", new JFXButton("nihao")));
        users.add(new User(SALES_DEPARTMENT, "25", "Employee 4", new JFXButton("nihao")));
        users.add(new User(SALES_DEPARTMENT, "27", "Employee 5", new JFXButton("nihao")));
        users.add(new User(IT_DEPARTMENT, "42", "ID 2", new JFXButton("nihao")));
        users.add(new User(HR_DEPARTMENT, "21", "HR 1", new JFXButton("nihao")));
        users.add(new User(HR_DEPARTMENT, "28", "HR 2", new JFXButton("nihao")));
        ...
        treeView.getColumns().setAll(deptColumn, ageColumn, empColumn, btnColumn);
    ...
    private static final class User extends RecursiveTreeObject<User> {
        ...
        final ObjectProperty btn;

        User(String department, String age, String userName, Object btn) {
            this.department = new SimpleStringProperty(department);
            this.userName = new SimpleStringProperty(userName);
            this.age = new SimpleStringProperty(age);
            this.btn = new SimpleObjectProperty(btn);
        }
    }
}
